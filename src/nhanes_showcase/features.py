...
def define_target(df: pd.DataFrame, source_col: str="DIQ010") -> pd.DataFrame:
    out = df.copy()
    out[TARGET_COL] = out[source_col].map({1: 1.0, 2: 0.0})
    return out
...
def get_model_feature_lists(df: pd.DataFrame, include_glucose: bool=False) -> tuple[list[str], list[str]]:
    numeric=["age_years","bmi","systolic_bp_mean","diastolic_bp_mean", "poverty_income_ratio"]
    categorical=["sex","race_ethnicity","education_level","age_group", "bmi_category"]
    if include_glucose and "fasting_glucose" in df.columns:
...
def select_model_data(df: pd.DataFrame, include_glucose: bool=False) -> tuple[pd.DataFrame, pd.Series, list[str], list[str], pd.DataFrame]:
