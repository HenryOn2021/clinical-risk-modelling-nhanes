...
def download_catalog(catalog: dict[str, str] | None = None, raw_dir: Path = RAW_DIR, overwrite: bool = False) -> dict[str, Path]:
    ensure_project_dirs()
...
def load_local_tables(raw_dir: Path = RAW_DIR, names: list[str] | None = None) -> dict[str, pd.DataFrame]:
...
def load_required_tables(overwrite: bool = False) -> dict[str, pd.DataFrame]:
    catalog = get_file_catalog()
