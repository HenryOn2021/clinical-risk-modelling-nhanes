...
from nhanes_showcase.stats import (
    cohort_summary,
...
def main() -> None:
    ...
    cohort_summary(df, weight_col=weight_col).to_csv(
        METRICS_DIR / "cohort_summary.csv", index=False
    )

    if "sex" in df.columns:
        summarise_subgroups(df, "sex", weight_col=weight_col).to_csv(
            METRICS_DIR / "subgroup_sex.csv", index=False
        )

    if "age_group" in df.columns:
        summarise_subgroups(df, "age_group", weight_col=weight_col).to_csv(
            METRICS_DIR / "subgroup_age_group.csv", index=False
        )

    if "race_ethnicity" in df.columns:
        summarise_subgroups(df, "race_ethnicity", weight_col=weight_col).to_csv(
            METRICS_DIR / "subgroup_race_ethnicity.csv", index=False
        )
