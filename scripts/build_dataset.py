...
def main() -> None:
    ...
    parser.add_argument(
        "--include-glucose", action="store_true", help="Keep fasting glucose as a feature"
    )
    ...
    download_catalog(catalog, overwrite=args.overwrite_downloads)
    tables = load_local_tables(names=list(catalog))
    ...
    print(f"Analysis cohort: {analysis.shape[0]:,} adults")
